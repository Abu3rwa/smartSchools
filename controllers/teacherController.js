...
const getTeacherCapacityContext = async (schoolId) => {
    const featureContext = await resolveSchoolFeatureContext(schoolId);
    const rawLimit = featureContext?.limits?.maxTeachers;
    const maxTeachers = Number(rawLimit);

    if (!Number.isFinite(maxTeachers) || maxTeachers < 0) {
        return {
            isLimited: false,
            maxTeachers: null,
            currentTeachers: await Teacher.countDocuments({ school: schoolId, isActive: true })
        };
    }

    const currentTeachers = await Teacher.countDocuments({ school: schoolId, isActive: true });
    return {
        isLimited: true,
        maxTeachers,
        currentTeachers,
        remainingSeats: Math.max(0, maxTeachers - currentTeachers)
    };
};

const buildTeacherLimitReachedMessage = ({ maxTeachers, currentTeachers }) => (
    `Teacher limit reached for your plan. You can have up to ${maxTeachers} active teachers and currently have ${currentTeachers}. Please request an upgrade to add more teachers.`
);
...
export const createTeacher = asyncHandler(async (req, res) => {
    ...
    const capacity = await getTeacherCapacityContext(req.schoolId);
    if (capacity.isLimited && capacity.currentTeachers >= capacity.maxTeachers) {
        return res.status(403).json({
            success: false,
            code: 'TEACHER_LIMIT_REACHED',
            message: buildTeacherLimitReachedMessage(capacity),
            data: {
                maxTeachers: capacity.maxTeachers,
                currentTeachers: capacity.currentTeachers,
                remainingSeats: 0
            }
        });
    }
