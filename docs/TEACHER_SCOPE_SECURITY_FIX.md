...
**department_principals:** See all classes (within their department scope). Admins see all classes. Only teachers are restricted to their assigned classes.

### Additional Fixes (Teacher-Only Class Access)

- **getClassStats** – Teachers must be assigned to the class to view stats.
- **Class analytics/insights** – `ensureClassAccess` now enforces department scope for department_principals; teachers remain restricted to assigned classes.
- **Newsletter endpoints** – `getNewsletterIssue` and `ensureNewsletterIssue` now verify teachers can only access newsletters for their assigned classes.
- **Grade getClassStatistics** – Teachers must be authorized for the class+subject to view stats.
- **Client classSlice** – Removed default `myClassesOnly` for department_principals; they fetch all classes like admins.
...
- `controllers/classController.js` – Teacher-only filtering in getClasses; teacher + department scope in getClassStats
- `controllers/classAnaliticsController.js` – Department scope in ensureClassAccess
- `controllers/gradeController.js` – Teacher check in getClassStatistics
- `controllers/newsletterController.js` – Teacher class restriction in getNewsletterIssue and ensureNewsletterIssue
- `routes/classRoutes.js` – Added department_principal to class routes; teachers restricted server-side
